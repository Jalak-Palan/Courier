...
/**
 * Shree Anjani Courier scraper
 * Site: https://shreeanjanicouriertracking.com/
 */
async function scrapeShreeAnjani(trackingId) {
  let page = null;
  try {
    page = await getPage();
    await withTimeout(
      page.goto('https://shreeanjanicouriertracking.com/', { waitUntil: 'domcontentloaded' }),
      20000
    );

    // Find tracking input
    await withTimeout(
      page.waitForSelector('input[type="text"], input[name*="awb"], input[name*="track"], input[id*="track"]', { timeout: 10000 }),
      12000
    );
    ...
    await Promise.all([
      ...
      page.click('button[type="submit"], input[type="submit"], .btn, .track-btn').catch(() => inputEl.press('Enter')),
    ]);

    // Wait for result
    await withTimeout(
      page.waitForSelector('table, .tracking-result, .track-result, .result, .status, [class*="track"]', { timeout: 15000 }),
      18000
    );
    ...
    const result = await page.evaluate(() => {
      ...
      status = status || getText(['.status', '[class*="status"]', 'h2', 'h3', 'strong']);
    ...
    return {
      ...
      courier: 'Shree Anjani',
    };
  } catch (err) {
    console.error('[Shree Anjani Scraper]', err.message);
    return null;
  } finally {
...
module.exports = { scrapeShreeAnjani };
